...
// Para evitar loops de cambios cuando aplicamos updates remotos
let SUPRIMIR_EMIT_FORM   = false;
let SUPRIMIR_EMIT_EVENTO = false;
...
function aplicarSnapshotInicial(data) {
  ...
  eventos.forEach(ev => {
    agregarFilaEvento(false, ev); // NO emitir
  });
}
...
/**
 * ✅ IMPORTANTE:
 * Antes eliminabas filas para calzar exacto con opIds.
 * Eso es lo que “te borra operadores”.
 *
 * Solución:
 * - Solo agregamos filas si faltan
 * - NO eliminamos filas existentes desde remoto
 * - Solo seteamos valores si vienen definidos
 */
function aplicarFormUpdateRemoto(data) {
  ...
  if (operadoresContainer && operadorTemplate) {
    ...
    // 2) Re-leer selects y asignar valores SOLO si existen en payload
    selects = operadoresContainer.querySelectorAll('select[name="operadores[]"]');
    selects.forEach((sel, idx) => {
      if (opIds[idx] !== undefined) {
        sel.value = opIds[idx] || "";
      }
    });
  }
...
function agregarFilaEvento(emitirPorSocket = true, rowData = null) {
  ...
  // ✅ Guard anti-duplicado: si ya existe esa fila, NO la vuelvas a crear
  const existente = eventosContainer.querySelector(`.input-group[data-row-id="${rowId}"]`);
  if (existente) return;
  ...
  // poblar lugares
  if (selectLugar) {
    if (lugaresDisponibles.length > 0) poblarSelectLugares(selectLugar);
    else selectLugar.innerHTML = '<option value="">Seleccione un cliente...</option>';
  }

  // Aplicar valores si vienen del servidor
  if (rowData) {
    if (selectLugar && rowData.lugar_id != null) selectLugar.value = String(rowData.lugar_id);
    if (inputCamara && rowData.camara != null)  inputCamara.value = rowData.camara;
    if (inputHora && rowData.hora != null)      inputHora.value = rowData.hora;
    if (selectTipo && rowData.tipo_evento_id != null) selectTipo.value = String(rowData.tipo_evento_id);
    if (inputObs && rowData.observaciones != null) inputObs.value = rowData.observaciones;
  }

  eventosContainer.appendChild(clone);

  // Si la fila la creó este usuario → avisar al servidor
  if (emitirPorSocket && window.socket && !SUPRIMIR_EMIT_EVENTO) {
...
function aplicarUpdateEventoRemoto(data) {
  ...
  let elem = null;
  switch (field) {
    case "lugar_id":       elem = row.querySelector('select[name="lugares[]"]'); break;
    case "camara":         elem = row.querySelector('input[name="camaras[]"]'); break;
    case "hora":           elem = row.querySelector('input[name="horas_evento[]"]'); break;
    case "tipo_evento_id": elem = row.querySelector('select[name="tipos_evento[]"]'); break;
    case "observaciones":  elem = row.querySelector('input[name="observaciones[]"]'); break;
  }

  if (elem) {
    SUPRIMIR_EMIT_EVENTO = true;
    elem.value = value != null ? value : "";
    SUPRIMIR_EMIT_EVENTO = false;
  }
}
...
// VACIAR UI
// ==========================
function vaciarBitacoraUI({ mantenerCliente = true } = {}) {
  if (eventosContainer) eventosContainer.innerHTML = "";

  if (operadoresContainer && operadorTemplate) {
    operadoresContainer.innerHTML = "";
    operadoresContainer.appendChild(operadorTemplate.content.cloneNode(true));
  }
  ...
  if (!mantenerCliente) {
    const cliente = document.querySelector('select[name="cliente_id"]') || document.getElementById("cliente");
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ====== CAMBIO DE CLIENTE → LUGARES (+ opcional reconectar socket) ======
  if (clienteSelect) {
    clienteSelect.addEventListener('change', async (e) => {
      const clienteId = e.target.value;
      currentClienteId = clienteId || null;

      // Si quieres reconectar socket cuando cambia cliente:
      if (typeof window.reconectarSocketCliente === "function") {
        window.reconectarSocketCliente(currentClienteId);
      }
      ...
      try {
        ...
        Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudieron cargar los lugares para este cliente.' });
      }
    });
  }
  ...
  if (operadoresContainer) {
    ...
    operadoresContainer.addEventListener('change', emitirFormUpdate);
  }
  ...
  if (eventosContainer) {
    ...
    eventosContainer.addEventListener('change', (e) => {
      if (SUPRIMIR_EMIT_EVENTO) return;
      ...
      if (e.target.name === "lugares[]") field = "lugar_id";
      else if (e.target.name === "camaras[]") field = "camara";
      else if (e.target.name === "horas_evento[]") field = "hora";
      else if (e.target.name === "tipos_evento[]") field = "tipo_evento_id";
      else if (e.target.name === "observaciones[]") field = "observaciones";

      if (!field) return;

      window.socket.emit("evento_update", { id: rowId, field, value });
    });
  }

  // ====== BOTÓN VACIAR (solo emite, el socket mostrará el Swal) ======
  const btnVaciar = document.getElementById("btnVaciarBitacora");
  btnVaciar?.addEventListener("click", () => {
    if (!window.socket || !window.socket.connected) {
      Swal.fire({ icon: "warning", title: "Sin conexión", text: "Socket no está conectado todavía." });
      return;
    }

    Swal.fire({
      title: "¿Vaciar bitácora?",
      text: "Esto eliminará eventos y datos del formulario en TODAS las pestañas del mismo cliente.",
      icon: "warning",
      ...
      if (r.isConfirmed) window.socket.emit("bitacora_vaciar", { motivo: "manual" });
    });
  });
  ...
  window.vaciarBitacoraUI = vaciarBitacoraUI;

  // ====== BOTÓN ANALIZAR BITÁCORA ======
  const btnAnalizar = document.getElementById("btnAnalizarBitacora");
  ...
  btnAnalizar?.addEventListener("click", () => {
    ...
    if (!currentClienteId) {
      Swal.fire({
        ...
        text: "Debe seleccionar un cliente antes de analizar la bitácora."
      });
      return;
    }

    if (!Array.isArray(lugaresDisponibles) || lugaresDisponibles.length === 0) {
      Swal.fire({
        icon: "warning",
        title: "Sin lugares",
        text: "No se han cargado los lugares del cliente."
      });
      return;
    }
    ...
    const filas = document.querySelectorAll('#eventos-container .input-group');

    filas.forEach(group => {
      const selLugar = group.querySelector('select[name="lugares[]"]');
      const val = selLugar ? selLugar.value : "";
      if (val) usados.add(String(val));
    });

    const faltantes = lugaresDisponibles
    ...
    if (faltantes.length === 0) {
      ...
      Swal.fire({
        ...
        text: "Todos los lugares del cliente tienen al menos un evento registrado.",
        timer: 2000,
    ...
    Swal.fire({
      icon: "warning",
      title: "Faltan lugares por registrar",
      html: `
        <div style="text-align:left">
          <p>Los siguientes lugares no tienen eventos:</p>
          <ul>
            ${faltantes.map(l => `<li>${l}</li>`).join("")}
          </ul>
        </div>
      `
    });
...
window.confirmarEnvio = function (event) {
  ...
  tipoEventoSelects.forEach(select => {
    if (!select.value) { select.classList.add('is-invalid'); error = true; }
  ...
  horaInputs.forEach(input => {
    if (!input.value) { input.classList.add('is-invalid'); error = true; }
    else input.classList.remove('is-invalid');
  });

  const camaraInputs = form.querySelectorAll('input[name="camaras[]"]');
  camaraInputs.forEach(input => {
    if (!input.value.trim()) { input.classList.add('is-invalid'); error = true; }
    else input.classList.remove('is-invalid');
  });
  ...
  if (!horarioSelect.value) { horarioSelect.classList.add('is-invalid'); error = true; }
  else horarioSelect.classList.remove('is-invalid');

  const fechaInicio = form.querySelector('input[name="fecha_inicio"]');
  const fechaFin    = form.querySelector('input[name="fecha_fin"]');

  if (!fechaInicio.value) { fechaInicio.classList.add('is-invalid'); error = true; }
  else fechaInicio.classList.remove('is-invalid');

  if (!fechaFin.value) { fechaFin.classList.add('is-invalid'); error = true; }
  else fechaFin.classList.remove('is-invalid');

  if (fechaInicio.value && fechaFin.value && fechaFin.value < fechaInicio.value) {
    fechaFin.classList.add('is-invalid');
    fechaInicio.classList.add('is-invalid');
    Swal.fire({ icon: 'error', title: 'Fechas inválidas', text: 'La fecha de finalización no puede ser anterior a la fecha de inicio.' });
    return;
  }

  if (error) {
    Swal.fire({ icon: 'error', title: 'Campos incompletos', text: 'Por favor, completa correctamente todos los campos requeridos antes de continuar.' });
    return;
  }
...
document.getElementById("excel-input")?.addEventListener("change", function (e) {
  ...
  reader.onload = function (event) {
    ...
    Swal.fire({ icon: "success", title: "Eventos cargados", text: `${eventos.length} eventos importados desde Excel.`, timer: 2000, showConfirmButton: false });
  };
...
function encontrarOpcionPorTexto(select, texto) {
  ...
  for (let option of select.options) {
    if (option.text.trim().toLowerCase() === texto) return option.value;
  }
  return "";
}
function analizarBitacoraFaltantesLugares() {
  // Debe existir cliente seleccionado y lugares cargados
  if (!currentClienteId) {
    return { ok: false, error: "Selecciona un cliente primero." };
  }

  if (!Array.isArray(lugaresDisponibles) || lugaresDisponibles.length === 0) {
    return { ok: false, error: "Aún no se cargan los lugares del cliente." };
  }

  // Lugares usados en eventos
  const usados = new Set();
  const filas = document.querySelectorAll('#eventos-container .input-group');
  ...
  // Faltantes = lugaresDisponibles - usados
  const faltantes = lugaresDisponibles
    .filter(l => !usados.has(String(l.id)))
    .map(l => l.nombre);
