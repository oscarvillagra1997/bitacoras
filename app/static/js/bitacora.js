...
// referencias DOM que usaremos en varios lados
...
function inicializarSocket() {
    try {
        // usamos cliente_id en la query para entrar a la sala correcta
        socket = io({
            query: {
        ...
        // Snapshot inicial cuando alguien entra tarde
        socket.on("snapshot_inicial", (data) => {
            console.log("📦 Snapshot recibido:", data);
            aplicarSnapshotInicial(data);
        });

        // otro usuario creó una fila nueva
        socket.on("evento_nuevo", (rowData) => {
            console.log("📢 evento_nuevo desde otro cliente:", rowData);
            agregarFilaEvento(false, rowData);
        });

        // otro usuario modificó un campo
        socket.on("evento_update", (data) => {
            aplicarUpdateEventoRemoto(data);
        });

        // otro usuario eliminó una fila
        socket.on("evento_eliminar", (data) => {
...
function emitirFormUpdate() {
    ...
    socket.emit("form_update", payload);
}
...
function aplicarFormUpdateRemoto(data) {
    ...
    if (operadoresContainer) {
        const selects = operadoresContainer.querySelectorAll('select[name="operadores[]"]');

        // si hay menos selects que operadores → podríamos crear más, pero para demo
        // solo sincronizamos los que existan.
...
function agregarFilaEvento(emitirPorSocket = true, rowData = null) {
    ...
    if (emitirPorSocket && socket) {
        socket.emit("evento_nuevo", {
            id: rowId,
            row: {
                id: rowId,
    ...
    if (eventosContainer) {
        eventosContainer.addEventListener('click', (e) => {
            if (e.target.classList.contains('remove-evento')) {
                ...
                if (socket && rowId) {
                    socket.emit("evento_eliminar", { id: rowId });
                }
            }
        });

        // cualquier cambio dentro de una fila
...
// ==========================
// (resto de funciones: exportarXLSXAntesDeEnviar, confirmarEnvio,
// importar desde Excel, encontrarOpcionPorTexto) → igual que los tenías
// ==========================

// ... (mantén tu código de exportarXLSX, confirmarEnvio e importar XLSX tal cual)
...
document.getElementById("excel-input")?.addEventListener("change", function (e) {
    ...
    reader.onload = function (event) {
        ...
        Swal.fire({
            ...
            text: `${eventos.length} eventos importidos desde Excel.`,
            timer: 2000,
